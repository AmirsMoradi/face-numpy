...
def predict_face_region(model, region):
    ...
    prediction = model.predict([resized.flatten()])[0]
    return prediction == 1
    ...
    for window_size in window_sizes:
        for y in range(0, gray.shape[0] - window_size[1], step_size):
            for x in range(0, gray.shape[1] - window_size[0], step_size):
                ...
                if region.shape[:2] == window_size:
                    is_face = predict_face_region(model, region)
                    if is_face:
                        print(f"Face detected at (x: {x}, y: {y}, w: {window_size[0]}, h: {window_size[1]})")
                        frame[y:y + window_size[1], x:x + 3] = [255, 255, 0]
                        frame[y:y + window_size[1], x + window_size[0] - 3:x + window_size[0]] = [255, 255,
                                                                                                  0]
                        frame[y:y + 3, x:x + window_size[0]] = [255, 255, 0]
                        frame[y + window_size[1] - 3:y + window_size[1], x:x + window_size[0]] = [255, 255,
                                                                                                  0]
